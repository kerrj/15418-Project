...
__global__ void _makeDistMatrix(const char* featureBuf1, const char* featureBuf2, int size1, int size2, FeatureDist* output){
	...
	output[x + size1 * y].rowIndex = y;
	output[x + size1 * y].colIndex = x;
...
void makeDistMatrix(char* featureBuf1, char* featureBuf2, int size1, int size2, FeatureDist *output){
	...
	_makeDistMatrix<<< gridDims, blockSize >>>(featureBuf1,featureBuf2,size1,size2,output);
}
